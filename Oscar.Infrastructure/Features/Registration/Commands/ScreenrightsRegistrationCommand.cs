...
{
    public class ScreenrightsRegistrationCommand : IRequest<Result<string>>
    {
        public Guid BatchId { get; set; }
        public int ClientId { get; set; }
    }

    public class ScreenrightsRegistrationCommandHandler : AbstractBaseHandler<ScreenrightsRegistrationCommand, string>
    {
        private IExporter _exporter;
        private IRegistrationService<RegistrationWorksScreenrightsExport> _registrationService;
        private readonly ConcurrentBag<string> _registrationErrors = new();

        public ScreenrightsRegistrationCommandHandler(
            OscarContext oscarContext,
            IMapper mapper,
            IValidator<ScreenrightsRegistrationCommand> validator,
            IExporter exporter,
            IRegistrationService<RegistrationWorksScreenrightsExport> registrationService,
            ILogger<ScreenrightsRegistrationCommand> logger)
        ...
        protected override async Task<Result<string>> HandleRequest(ScreenrightsRegistrationCommand request, CancellationToken cancellationToken)
        {
            var strategy = OscarContext.Database.CreateExecutionStrategy();
            Result<RegistrationWorksScreenrightsExport> registrationResult = null;
