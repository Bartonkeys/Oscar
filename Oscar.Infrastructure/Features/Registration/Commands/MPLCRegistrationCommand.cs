...
{
    public class MPLCRegistrationCommand : IRequest<Result<string>>
    {
        public Guid BatchId { get; set; }
        public int ClientId { get; set; }
    }

    public class MPLCRegistrationCommandHandler : AbstractBaseHandler<MPLCRegistrationCommand, string>
    {
        private IExporter _exporter;
        private IRegistrationService<RegistrationWorksMPLCDto> _registrationService;
        private readonly ConcurrentBag<string> _registrationErrors = new();

        public MPLCRegistrationCommandHandler(
            OscarContext oscarContext,
            IMapper mapper,
            IValidator<MPLCRegistrationCommand> validator,
            IExporter exporter,
            IRegistrationService<RegistrationWorksMPLCDto> registrationService,
            ILogger<MPLCRegistrationCommand> logger)
        ...
        protected override async Task<Result<string>> HandleRequest(MPLCRegistrationCommand request, CancellationToken cancellationToken)
        {
            var strategy = OscarContext.Database.CreateExecutionStrategy();
            Result<RegistrationWorksMPLCDto> registrationResult = null;
