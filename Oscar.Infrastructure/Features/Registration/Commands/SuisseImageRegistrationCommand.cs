...
{
    public class SuisseImageCommand : IRequest<Result<string>>
    {
        public Guid BatchId { get; set; }
        public int ClientId { get; set; }
    }

    public class SuisseImageCommandHandler: AbstractBaseHandler<SuisseImageCommand, string>
    {
        private IExporter _exporter;
        private IRegistrationService<RegistrationWorksSuissImageExport> _registrationService;
        private readonly ConcurrentBag<string> _registrationErrors = new();

        public SuisseImageCommandHandler(
            OscarContext oscarContext,
            IMapper mapper,
            IValidator<SuisseImageCommand> validator,
            IExporter exporter,
            IRegistrationService<RegistrationWorksSuissImageExport> registrationService,
            ILogger<SuisseImageCommand> logger)
        ...
        protected override async Task<Result<string>> HandleRequest(SuisseImageCommand request, CancellationToken cancellationToken)
        {
            var strategy = OscarContext.Database.CreateExecutionStrategy();
            Result<RegistrationWorksSuissImageExport> registrationResult = null;
            ...
            await OscarContext.SaveChangesAsync(cancellationToken);
            return Result.Ok(registrationBatch.FileName);
        }
    }
}
