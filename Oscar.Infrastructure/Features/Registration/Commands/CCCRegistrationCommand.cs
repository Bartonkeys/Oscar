...
{
    public class CCCRegistrationCommand : IRequest<Result<string>>
    {
        public Guid BatchId { get; set; }
        public int ClientId { get; set; }
    }

    public class CCCRegistrationCommandHandler : AbstractBaseHandler<CCCRegistrationCommand, string>
    {
        private IExporter _exporter;
        private IRegistrationService<RegistrationWorksCCCDto> _registrationService;
        private readonly ConcurrentBag<string> _registrationErrors = new();

        public CCCRegistrationCommandHandler(
            OscarContext oscarContext,
            IMapper mapper,
            IValidator<CCCRegistrationCommand> validator,
            IExporter exporter,
            IRegistrationService<RegistrationWorksCCCDto> registrationService,
            ILogger<CCCRegistrationCommand> logger)
        ...
        protected override async Task<Result<string>> HandleRequest(CCCRegistrationCommand request, CancellationToken cancellationToken)
        {
            var strategy = OscarContext.Database.CreateExecutionStrategy();
            Result<RegistrationWorksCCCDto> registrationResult = null;
