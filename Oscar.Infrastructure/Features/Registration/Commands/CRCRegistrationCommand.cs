...
{
    public class CRCRegistrationCommand : IRequest<Result<string>>
    {
        public Guid BatchId { get; set; }
        public int ClientId { get; set; }
    }

    public class CRCRegistrationCommandHandler : AbstractBaseHandler<CRCRegistrationCommand, string>
    {
        private IExporter _exporter;
        private IRegistrationService<RegistrationWorksCRCDto> _registrationService;
        private readonly ConcurrentBag<string> _registrationErrors = new();

        public CRCRegistrationCommandHandler(
            OscarContext oscarContext,
            IMapper mapper,
            IValidator<CRCRegistrationCommand> validator,
            IExporter exporter,
            IRegistrationService<RegistrationWorksCRCDto> registrationService,
            ILogger<CRCRegistrationCommand> logger)
        ...
        protected override async Task<Result<string>> HandleRequest(CRCRegistrationCommand request, CancellationToken cancellationToken)
        {
            var strategy = OscarContext.Database.CreateExecutionStrategy();
            Result<RegistrationWorksCRCDto> registrationResult = null;
