...
{
    public class EGEDARegistrationCommand : IRequest<Result<string>>
    {
        public Guid BatchId { get; set; }
        public int ClientId { get; set; }
    }

    public class EGEDARegistrationCommandHandler : AbstractBaseHandler<EGEDARegistrationCommand, string>
    {
        private IExporter _exporter;
        private IRegistrationService<RegistrationWorksEGEDADto> _registrationService;
        private readonly ConcurrentBag<string> _registrationErrors = new();

        public EGEDARegistrationCommandHandler(
            OscarContext oscarContext,
            IMapper mapper,
            IValidator<EGEDARegistrationCommand> validator,
            IExporter exporter,
            IRegistrationService<RegistrationWorksEGEDADto> registrationService,
            ILogger<EGEDARegistrationCommand> logger)
        ...
        protected override async Task<Result<string>> HandleRequest(EGEDARegistrationCommand request, CancellationToken cancellationToken)
        {
            var strategy = OscarContext.Database.CreateExecutionStrategy();
            Result<RegistrationWorksEGEDADto> registrationResult = null;
