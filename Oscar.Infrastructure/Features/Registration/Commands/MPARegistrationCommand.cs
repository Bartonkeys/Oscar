...
{
    public class MPARegistrationCommand : IRequest<Result<string>>
    {
        public Guid BatchId { get; set; }
        public int ClientId { get; set; }
    }

    public class MPARegistrationCommandHandler : AbstractBaseHandler<MPARegistrationCommand, string>
    {
        private IExporter _exporter;
        private IRegistrationService<RegistrationWorksMPADto> _registrationService;
        private readonly ConcurrentBag<string> _registrationErrors = new();

        public MPARegistrationCommandHandler(
            OscarContext oscarContext,
            IMapper mapper,
            IValidator<MPARegistrationCommand> validator,
            IExporter exporter,
            IRegistrationService<RegistrationWorksMPADto> registrationService,
            ILogger<MPARegistrationCommand> logger)
        ...
        protected override async Task<Result<string>> HandleRequest(MPARegistrationCommand request, CancellationToken cancellationToken)
        {
            var strategy = OscarContext.Database.CreateExecutionStrategy();
            Result<RegistrationWorksMPADto> registrationResult = null;
