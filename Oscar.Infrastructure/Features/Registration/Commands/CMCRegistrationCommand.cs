...
{
    public class CMCRegistrationCommand : IRequest<Result<string>>
    {
        public Guid BatchId { get; set; }
        public int ClientId { get; set; }
    }

    public class CMCRegistrationCommandHandler : AbstractBaseHandler<CMCRegistrationCommand, string>
    {
        private IExporter _exporter;
        private IRegistrationService<RegistrationWorksCMCDto> _registrationService;
        private readonly ConcurrentBag<string> _registrationErrors = new();

        public CMCRegistrationCommandHandler(
            OscarContext oscarContext,
            IMapper mapper,
            IValidator<CMCRegistrationCommand> validator,
            IExporter exporter,
            IRegistrationService<RegistrationWorksCMCDto> registrationService,
            ILogger<CMCRegistrationCommand> logger)
        ...
        protected override async Task<Result<string>> HandleRequest(CMCRegistrationCommand request, CancellationToken cancellationToken)
        {
            var strategy = OscarContext.Database.CreateExecutionStrategy();
            Result<RegistrationWorksCMCDto> registrationResult = null;
