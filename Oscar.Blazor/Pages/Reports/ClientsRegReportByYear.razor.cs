...
{
    public partial class ClientsRegReportByYear : OscarReportComponent
    {
        #region - Private Declarations -
        private string _selectedYear;
        private ClientDto? _selectedClient;
        private MudAutocomplete<ClientDto>? _clientSelect;
        private MudDataGrid<ClientWorkStatItemEx>? _dataGrid;
        private List<ClientWorkStatItemEx>? _gridItems = new List<ClientWorkStatItemEx>();
        private ClientWorkStatItemEx? _selectedItem = null;
        #endregion
        ...
        #region - Grid Filters -
        private bool GridFilterFunc(ClientWorkStatItemEx item) 
        {
        ...
        {
            ReportName = nameof(ClientsRegReportByYear);
            ReportTitle = "Clients Registrations KPI Report by Production Year";
        ...
        {
            var items = await ReportService.GetClientProductionYearlyStats(GetUserQuery());
            if (items is { } data)
            {
        ...
        {
            return await ReportService.ExportClientWorkYearlyStats(ExportType.Excel, true, int.Parse(SelectedYear), SelectedClient?.Id, ReportName);
        }

        public override async Task<FileStreamResult> ExportReportToCsv()
        {
            return await ReportService.ExportClientWorkYearlyStats(ExportType.Csv, true, int.Parse(SelectedYear), SelectedClient?.Id, ReportName);
        }
        ...
        {
            if (string.IsNullOrWhiteSpace(SelectedYear)) return $"You must select Production Year to proceed.";
        ...
        private Query GetUserQuery()
        {
            ...
            if (!string.IsNullOrWhiteSpace(SelectedYear))
            {
                ...
                query.AddOrAppendFilter(LogicalFilterOperator.And, $"ProductionYear IN ({selectedYear}, {selectedYear - 1}, {selectedYear - 2})");
            }
