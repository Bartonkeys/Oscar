...
export default function SeriesDetails({open, id, toggleDrawer, refreshList}) {
    defaultSeries.id = id;
    ...
    const refreshWork = async() => {
        setFetching(true);
        setFetched(false);
        let retrievedWork = await get(`series/${id}`);
    ...
    const saveWork = async(successPage) => {
        try {
            ...
            await update('series/' + work.id, work) :
            await create('series', work);

            const action = id === 0? 'created': 'updated';
            setSaving(false);
            toastr('success', `Series ${action} successfully`);
            onClose();
        }
        catch (err) {
            setSaving(false);
            if(err.response && err.response.status === 400 && err.response.data && err.response.data.errorMessage){
              showValidationError(err.response.data.errorMessage);
                ...
                toastr('error', `Error updating series`);
            }
        }
    }
    ...
    return (
        <RightSlider canSave={true} title={id === 0? "Create Series": "Series Details"} onSave={saveWork}
